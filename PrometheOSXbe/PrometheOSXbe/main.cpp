...
utils::dataContainer* onGetCallback(const char* path, const char* query)
{
	...
	else if (stringUtility::equals(path, "\\api\\cerbiosini", true))
	{
		...
		cerbiosConfig config = cerbiosIniHelper::loadConfig("HDD0-C:\\cerbios.ini", CerbiosVersionLegacy, NULL);
		cerbiosIniHelper::buildConfig(&config, CerbiosVersionLegacy, NULL, temp);
